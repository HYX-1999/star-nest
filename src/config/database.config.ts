...
export default async (configService: ConfigService): Promise<TypeOrmModuleOptions> => ({
  ...
  password: configService.get<string>('DATABASE_PASSWORD', 'admin5698'), // 默认值：password
  database: configService.get<string>('DATABASE_NAME', 'blog'), // 默认值：mydb
